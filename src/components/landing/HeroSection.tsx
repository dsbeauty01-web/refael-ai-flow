    ...
    <section className="relative min-h-screen flex items-center overflow-hidden pt-16">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full grid lg:grid-cols-2 gap-12 items-center">
        <div className={isHebrew ? 'text-right' : 'text-left'} dir={isHebrew ? 'rtl' : 'ltr'}>
          <span className="inline-block gradient-coral text-white text-[0.85rem] font-bold px-5 py-2 rounded-full mb-6 shadow-lg glow-coral">
            {isHebrew ? 'לעסקים קטנים' : 'FOR SMALL BUSINESSES'}
          </span>

          <h1 className={`text-[3.75rem] sm:text-[4.5rem] lg:text-[5.25rem] font-black text-foreground mb-6 leading-[1.05] tracking-tight ${isHebrew ? 'font-hebrew' : ''}`}>
            {isHebrew ? (
          ...
          <p className={`text-[1.25rem] text-muted-foreground mb-10 leading-relaxed max-w-xl ${isHebrew ? 'font-hebrew' : ''}`}>
            {isHebrew
          ...
          <div className={`flex flex-col sm:flex-row gap-3 ${isHebrew ? 'justify-end' : 'justify-start'}`}>
            <Button
              size="lg"
              className="gradient-coral text-white font-bold text-[1.1rem] h-14 px-10 rounded-full hover:opacity-95 hover:scale-[1.03] transition-all shadow-xl glow-coral animate-pulse-glow"
            ...
            <Button
              size="lg"
              variant="outline"
              className="glass border-white/15 text-foreground hover:bg-white/10 hover:border-white/25 font-bold text-[1.1rem] h-14 px-10 rounded-full transition-all"
        ...
        <div className="relative flex items-center justify-center h-[500px] lg:h-[650px]">
          <div className="absolute w-[420px] h-[420px] rounded-full border-2 border-primary/30 animate-pulse-ring" />
          <div className="absolute w-[520px] h-[520px] rounded-full border border-accent/20 animate-pulse-ring" style={{ animationDelay: '0.5s' }} />
          <div className="absolute w-[600px] h-[600px] rounded-full border border-white/5 animate-pulse-ring" style={{ animationDelay: '1s' }} />
          ...
          <div className="relative z-10 animate-float-y">
            <div className="w-[400px] h-[400px] rounded-full overflow-hidden shadow-2xl border-4 border-white/15 glow-blue bg-gradient-to-br from-primary/30 to-accent/30">
              <img src={heroAvatar} alt="AI Assistant" width={1024} height={1024} className="w-full h-full object-cover object-top scale-105" />
            </div>
            <div className="absolute inset-0 rounded-full -z-10 blur-3xl opacity-70 bg-gradient-to-br from-primary/60 to-accent/60" />
          </div>
